...
function verifyAccessToken(token: string) {
  try {
    ...
    const data = jwt.verify(token, JWT_SECRET_ACCESS_TOKEN as string)
    return { data, message: 'Token sudah diverifikasi' }
  } catch (err) {
    if (err instanceof TokenExpiredError) {
      return { data: null, message: `Session anda telah berakhir` }
    }
...
export { getToken, currentToken, verifyAccessToken }
